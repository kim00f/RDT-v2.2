    ...
    def rdt_send(self, process_buffer):
        ...
        for data in process_buffer:
            ...
            while (not RDTSender.is_expected_seq(reply,self.sequence) or RDTSender.is_corrupted(reply)):
                pkt3=RDTSender.clone_packet(pkt)
                print('Sender expecting sequence number: ' + pkt3['sequence_number'])
                print('Sender sending:{ Sequence number : ' + pkt3['sequence_number'] + ' data :' +  data + ' checksum: ' +  str(pkt3['checksum']) + '}')
                print('Reciever expecting sequence number: ' + pkt3['sequence_number'])
                reply =self.net_srv.udt_send(pkt3)
            if(self.sequence=='0'):
                self.sequence ='1'
